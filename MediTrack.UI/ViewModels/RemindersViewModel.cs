...
public partial class RemindersViewModel : ObservableObject
{
    ...
    [ObservableProperty]
    private ReminderSetting _selectedReminder = new();
    ...
    public ObservableCollection<int> SnoozeOptions { get; } = new()
    { 5, 10, 15, 30, 60 };
    ...
    private ObservableCollection<NavItem> _navItems = new()
    {
        ...
        new NavItem { Label = "Reminders", Icon = "M18 16v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2zm-5 0h-2v-2h2v2zm0-4h-2V9h2v3z", IsSelected = true },
    ...
    private async Task LoadRemindersAsync()
    {
        ...
        int id = 1;
        foreach (var med in meds.Where(m => !string.IsNullOrWhiteSpace(m.ReminderTimes) && !m.IsArchived))
        {
    ...
    [RelayCommand]
    private async Task DeleteReminderAsync(ReminderSetting reminder)
    {
        if (reminder.MedicationId == null) return;
        var med = await _medicationService.GetByIdAsync(reminder.MedicationId.Value);
        if (med == null) return;

        med.ReminderTimes = string.Empty;
        await _medicationService.UpdateAsync(med);
    ...
    [RelayCommand]
    private void ToggleReminder(ReminderSetting reminder)
    {
        reminder.IsEnabled = !reminder.IsEnabled;
        var current = Reminders.ToList();
        Reminders = new ObservableCollection<ReminderSetting>(current);
    }
